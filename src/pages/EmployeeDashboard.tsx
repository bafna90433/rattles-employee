...
const EmployeeDashboard:React.FC=()=>{
 ...
 ]);const dashboardSettings=await (window as any).electronAPI.getDashboardSettings?.().catch(()=>null);setSettings(dashboardSettings||{});setData({raw:raw||[],prod:prod||[],packet:packet||[],sales:sales||[],rawStock:rawStock||[],prodStock:prodStock||[],packetStock:packetStock||[]});setLoading(false)},[]);
 useEffect(()=>{load()},[load]);
 ...
 const workflowImages=(settings.workflowImages||[]).reduce((acc:any,img:any)=>({...acc,[img.key]:img.image_url}),{});
 const workflowImage=(key:string)=>workflowImages[key]||workflowFallbacks[key];
 const stations=[
  {num:1,title:"Materials",sub:"Raw Material Entry",img:workflowImage("materials"),value:n(x.raw),unit:"parts in stock",status:"Live",button:"Start Entry",path:"/employee/raw-add",tone:"red"},
  {num:2,title:"Rattle Build",sub:"Product Production",img:workflowImage("production"),value:n(x.products),unit:"rattles produced",status:"Live",button:"Start Production",path:"/employee/production-list",tone:"gold"},
  {num:3,title:"Packet Assembly",sub:"Packet Production",img:workflowImage("packing"),value:n(x.packets),unit:"packets available",status:"Live",button:"Start Assembly",path:"/employee/packet-production",tone:"teal"},
  {num:4,title:"Dispatch / Sales",sub:"Packet Sale",img:workflowImage("dispatch"),value:n(x.sales),unit:"packets sold",status:"Ready",button:"Start Dispatch",path:"/employee/packet-sales",tone:"orange"}];
